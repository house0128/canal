...
public class ESTemplate {
     ...
     * @return
     */
    public boolean updateByRightQuery(ESSyncConfig config, Map<String, Object> paramsTmp,Map<String,Object> paramSql, Map<String, Object> esFieldData) {
        ...
        esFieldData.forEach((key, value) -> {
            if (value instanceof Map) {
                ...
                sb.append("ctx._source").append("['").append(key).append("']").append(" = ").append(value).append("L;");
        ...
        if (response.getStatus().getVersionConflicts() > 0) {
            if(config.getEsMapping().isOptimisticLockReverseSearch()){
                logger.warn("本次updateByQuery存在乐观锁版本冲突，准备反查数据库重新执行..."+mapping.get_index()+" "+esFieldData);
                updateBySearchSql(config, paramSql, esFieldData);
            }else{
                try {
                ...
                logger.warn("本次updateByQuery存在乐观锁版本冲突，等待1s重新插入..."+mapping.get_index()+" "+esFieldData);
                return updateByRightQuery(config,  paramsTmp,paramSql, esFieldData);
            }
     ...
     * @return
     */
    private boolean updateBySearchSql(ESSyncConfig config, Map<String, Object> paramsSql, Map<String, Object> esFieldData) {
        ESMapping mapping = config.getEsMapping();
        ...
        if(paramsSql.size()==0){
            logger.error("sql param size 0,sql:"+mapping.getSql());
            throw new RuntimeException("updateBySearchSql sql param size is 0");
        }
        // 查询sql更新
        ...
        paramsSql.forEach(
                (fieldName, value) -> sql.append(fieldName).append("=").append(value).append(" AND "));
        int len = sql.length();
    ...
    public Object getESDataFromDmlData(ESMapping mapping, Map<String, Object> dmlData, Map<String, Object> dmlOld,
        ...
        //处理两张表字段同名的bug
        String aliceTableName = mapping.getSchemaItem().getAliasesName(tableName);
        if (StringUtils.isNotEmpty(aliceTableName)) {
